...
class ___{
    constructor(data){
        ...
        if(!this.res) alert("wwli: res is null: " + data);
...
__.setCookie = function(cName, cValue, expMin){
	let date = new Date();
	date.setTime(date.getTime() + (expMin * 60 * 1000));//min
	const expires = "expires=" + date.toUTCString();
	document.cookie = cName + "=" + cValue + "; " + expires + "; path=/;" + "SameSite=Strict";
}

__.deleteCookie = function(cName){
